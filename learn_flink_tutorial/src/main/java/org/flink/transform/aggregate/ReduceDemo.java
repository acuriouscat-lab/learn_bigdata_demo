...
public class ReduceDemo {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<WaterSensor> reduce = sensorKS.reduce(new ReduceFunction<WaterSensor>() {
            @Override
            public WaterSensor reduce(WaterSensor preSensor, WaterSensor curSensor) throws Exception {
                System.out.println("value1=" + preSensor);
                System.out.println("value2=" + curSensor);
                return new WaterSensor(preSensor.id, curSensor.ts, preSensor.vc + curSensor.vc);
            }
        });
